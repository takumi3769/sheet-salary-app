...
def get_worksheet(sh, month_str):
    try:
        ...
        worksheet = sh.add_worksheet(title=month_str, rows="100", cols="11")
        # 自動作成されるヘッダー（K列まで）
    ...
    for s in sh_main.worksheets():
        if "-" in s.title:
            ...
            if content:
                ...
                t_col = '給料合計' if '給料合計' in tdf.columns else '給料'
                if t_col in tdf.columns:
                    val = pd.to_numeric(tdf[t_col], errors='coerce').sum()
                    summary.append({"月": s.title, "支給額": f"{int(val):,}円"})
    if summary:
